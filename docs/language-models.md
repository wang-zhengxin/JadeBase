...
对话输入框可开启“深度思考”。JadeBase 会通过 SSE 先返回分析状态，再返回最终回答和可折叠
的思考摘要。兼容接口若返回 `reasoning_content`、`reasoning` 或 `reasoning_details`，页面会展示
模型提供的推理摘要；不支持独立推理字段的模型则展示向量召回、关键词召回、融合与重排结果的
可审计摘要。通义 Qwen3/QwQ 会自动附加 `enable_thinking`，OpenAI 推理模型会附加中等
`reasoning_effort`。思考摘要、耗时和步骤数随助手消息持久化，重新打开历史对话仍可查看。
