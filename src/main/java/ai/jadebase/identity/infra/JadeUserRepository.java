...
public interface JadeUserRepository extends JpaRepository<JadeUser, UUID> {
    ...
    List<JadeUser> findAllByOrderByUpdatedAtDesc();
    long countByStatus(JadeUser.Status status);
    long countByRoleAndStatus(JadeUser.Role role, JadeUser.Status status);
}
