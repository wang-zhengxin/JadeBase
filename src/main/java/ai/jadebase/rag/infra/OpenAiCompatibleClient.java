...
public class OpenAiCompatibleClient implements ChatModelClient {
    ...
    public String answer(String question, List<RetrievedChunk> context) {
        if (!configured()) return fallbackAnswer(context);
        ...
        Map<String, Object> body = Map.of(
                ...
                "temperature", 0.2,
                "messages", List.of(
                        Map.of("role", "system", "content", "你是企业知识助手。只依据提供的资料回答；资料不足时明确说明。引用事实时使用[资料N]标记。"),
    ...
    private String fallbackAnswer(List<RetrievedChunk> context) {
        if (context.isEmpty()) return "知识库中没有找到足够相关的内容。";
        StringBuilder answer = new StringBuilder("当前未配置大模型，以下是知识库中最相关的内容：\n\n");
