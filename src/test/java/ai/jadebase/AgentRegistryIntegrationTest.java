    ...
    void managesVersionedAgentsAndExecutesPublishedConfiguration() throws Exception {
        ...
        String input = """
                {"name":"Java 代码审查助手","description":"检查 Java 与 Spring 代码风险",
                 ...
                 "conversationStarters":["审查这段控制器代码","检查事务边界"],
                 "useKnowledge":true,"knowledgeBaseId":"%s","accessLevel":"EVERYONE",
                 "featured":true,"labels":["Java","代码审查"],
                 "enabledActions":["OPEN_URL","CODING_AGENT"],"knowledgeCutoffDate":"2025-12-31",
                 "thinkMode":true,"maxIterations":4}
        ...
        MvcResult created = mockMvc.perform(post("/api/v1/admin/agents").cookie(owner)
                ...
                .andExpect(jsonPath("$.conversationStarters[0]").value("审查这段控制器代码"))
                .andExpect(jsonPath("$.labels[1]").value("代码审查"))
                .andExpect(jsonPath("$.enabledActions[1]").value("CODING_AGENT"))
                .andReturn();
        ...
        mockMvc.perform(get("/api/v1/agents").cookie(member))
                ...
                .andExpect(jsonPath("$[0].conversationStarters.length()").value(2));
        ...
        ((com.fasterxml.jackson.databind.node.ObjectNode) changed).put("accessLevel", "PRIVATE");
        ...
        MvcResult directCreated = mockMvc.perform(post("/api/v1/admin/agents").cookie(owner)
                        .contentType(MediaType.APPLICATION_JSON).content("""
                                {"name":"通用分析助手","description":"不依赖知识库直接分析问题",
                                 "conversationStarters":["拆解这个问题"],"useKnowledge":false,
                                 "accessLevel":"EVERYONE","thinkMode":true,"maxIterations":3}
                                """))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.knowledgeBaseId").doesNotExist())
                .andExpect(jsonPath("$.knowledgeBaseName").value("未使用知识库"))
                .andReturn();
        String directAgentId = json.readTree(directCreated.getResponse().getContentAsString()).path("id").asText();
        mockMvc.perform(post("/api/v1/admin/agents/{id}/publish", directAgentId).cookie(owner))
                .andExpect(status().isOk()).andExpect(jsonPath("$.currentVersion").value(1));
        mockMvc.perform(post("/api/v1/chat").cookie(member).contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"knowledgeBaseId":"%s","agentId":"%s","question":"分析这个需求","thinkMode":false}
                                """.formatted(knowledgeBaseId, directAgentId)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.sources.length()").value(0))
                .andExpect(jsonPath("$.reasoning").value("已根据 Agent 指令直接分析问题，本次未启用知识库检索。"));
        mockMvc.perform(delete("/api/v1/admin/agents/{id}", directAgentId).cookie(owner))
